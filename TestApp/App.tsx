  ...
  try {
    ...
    const customCategoryName = 'Custom Category';
    {
      ...
      const customCategoryItem =
        StartScreenApi.JumpListItem.createWithArguments(
          'custom1',
          `Updated at ${date.toTimeString()} on ${date.toDateString()}`,
        );
      customCategoryItem.groupName = customCategoryName;
      customCategoryItem.logo = new Windows.Foundation.Uri(
...

    {
      const customCategoryItem =
        StartScreenApi.JumpListItem.createWithArguments(
          'custom2',
          'Proof JumpList is still updating!',
        );
      customCategoryItem.description =
        'and that IAsyncOperation and IVector are reflected correctly!';
      customCategoryItem.groupName = customCategoryName;
      customCategoryItem.logo = new Windows.Foundation.Uri(
        'ms-appx:///Assets/code.png',
      );
      items.append(customCategoryItem);
    }

    {
      const customCategoryItem =
        StartScreenApi.JumpListItem.createWithArguments(
          'custom3',
          'We should handle app launcher args too',
        );
      customCategoryItem.description =
        'to prove that JumpList will work fully end-to-end beyond WinRT projections.';
      customCategoryItem.groupName = customCategoryName;
      customCategoryItem.logo = new Windows.Foundation.Uri(
        'ms-appx:///Assets/video.png',
      );
      items.append(customCategoryItem);
    }

    {
      const taskItem = StartScreenApi.JumpListItem.createWithArguments(
        'task1',
        'Task 1',
      );
      taskItem.description = 'Still visible when recent is disabled by policy.';
      taskItem.logo = new Windows.Foundation.Uri(
        'ms-appx:///Assets/folder.png',
      );
...

    {
      ...
      taskItem.description =
        'Also visible when user disables the setting for recent items.';
...
export async function createBase64DataUriFromRandomAccessStreamAsync(
  ...
  if (dataSize > 0) {
    let contentType = (
      stream as Windows.Storage.Streams.IRandomAccessStreamWithContentType
    ).contentType;
    if (!contentType) {
      contentType = contentTypeHint || '';
    }

    const buffer = await stream.readAsync(
      new Windows.Storage.Streams.Buffer(dataSize),
      dataSize,
...
const App = () => {
  ...
  const loggingChannel = new Windows.Foundation.Diagnostics.LoggingChannel(
    'SampleProvider',
    null,
  );
      ...
      <SafeAreaView style={styles.root}>
        <ScrollView
          ...
          <View style={styles.body}>
            <View style={styles.sectionContainer}>
              <Text style={styles.sectionTitle}>
                Windows.UI.StartScreen.JumpList Example
              </Text>
              <Text style={styles.sectionDescription}>
                Right click the app in Start or Taskbar to see the populated
                JumpList.
              </Text>
            </View>
          </View>
          <View style={styles.body}>
            <View style={styles.sectionContainer}>
              <Text style={styles.sectionTitle}>
                Windows.Foundation.Diagnostics.LoggingChannel Example
              </Text>
              <Text style={styles.sectionDescription} selectable={true}>
                Click the following buttons to log events. These events can be
                seen in different ETW logging tools such as{' '}
                <Text
                  style={{color: 'blue', textDecorationLine: 'underline'}}
                  accessibilityRole="link"
                  onPress={() =>
                    Linking.openURL(
                      'https://docs.microsoft.com/windows/uwp/debug-test-perf/device-portal#event-tracing-for-windows-etw-logging',
                    )
                  }>
                  Device Portal
                </Text>{' '}
                ...
                <Text
                  ...
                  eff1e128-4903-5093-096a-bdc29b38456f
                </Text>
              </Text>
              <View
                style={{
                ...
                <Pressable
                  style={[
                    {marginHorizontal: 10},
                    styles.sectionDescriptionButton,
                  ]}
                  onPress={() => {
                    loggingChannel.logMessage(
                      'This string is being logged from an RNW application!',
                    );
                  }}>
                ...
                <Pressable
                  ...
                  onPress={() => {
                    loggingChannel.logValuePair(
                      'This value is being logged from an RNW application!',
                      42,
                    );
                  }}>
                  <Text style={styles.sectionDescriptionButtonText}>
          ...
          <View style={styles.body}>
            <View style={styles.sectionContainer}>
              ...
              <View
                ...
                <Text style={[{paddingRight: 10}, styles.sectionDescription]}>
